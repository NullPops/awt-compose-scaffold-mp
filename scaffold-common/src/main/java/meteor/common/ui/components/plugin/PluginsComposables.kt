        ...
        Column(Modifier.fillMaxSize()) {
            ...
            val favorites : List<Plugin> = nonHiddenPlugins.filter {
                if (!favoritesMap.containsKey(it)) {
                    favoritesMap[it] = Properties.get<Boolean>("plugin.${it.name}.isFavorite", false)
                }
            ...
            left = @Composable {
                Box(Modifier.padding(all = 2.dp).size(30.dp)) {
                    favoritesMap.putIfAbsent(
                        plugin,
                        Properties.get<Boolean>("plugin.${plugin.name}.isFavorite", false)
                    )
                    ...
                    if (isFavorite)
                        Image(
                            ...
                            modifier = Modifier.align(Alignment.Center).clickable {
                                ...
                                Properties.set("plugin.${plugin.name}.isFavorite", favoritesMap[plugin]!!)
                            })
                    else
                        Image(
                            LineAwesomeIcons.StarSolid,
            ...
            right = @Composable {
                ...
                if (!plugin.cantDisable) {
                    Switch(
                        ...
                        onCheckedChange = {
                            Properties.set("plugin.${plugin.name}.enabled", !runningMap[plugin]!!)
                            if (plugin.running) {
                                plugin.stop()
                            } else
                                plugin.start()
